...
코코아 프레임워크에 멀티 스레드 사용을 알려주기 위해서는 _NSThread_ 클래스를 사용해서 즉시 종료되는 스레드를 하나 생성하는 것입니다. 이 스레드의 엔트리 포인트는 아무것도 할 필요가 없으며 _NSThread_는 그저 스레드를 생성하는 동작을 하는 것만으로 코코아 프레임워크에 lock이 필요하다는 것을 알려줄 수 있습니다.
